...
# Linear Response imports
from src.utils.linear_response_utils import community_detection_synthetic
import matplotlib.pyplot as plt
from src.utils.linear_response_utils.run_utils import *
...
class Linear_Response(pl.LightningModule):
    def __init__(
        self,
        lag: int,
        num_nodes: int,
...
class Linear_Response_Trainer(pl.LightningModule):
    ...
    def __init__(
        ...
        self.latitudes, self.longitudes = community_detection_synthetic.generate_lat_lon_matrices(nx,ny)
        # Visualization Param:
        ...
        self.data_string = f'linear_response_numnodes_{self.num_nodes}_size_{self.nx}'
    ...
    def model_step(
        ...
        breakpoint()
        community_map, single_communities, average_signals, cumulative_signals = community_detect(data_v, heuristic = True)
        response_matrix, s_minus, s_plus = causality_linear_response(average_signals)

        null_response_high_percentile = s_plus
        null_response_low_percentile = s_minus
        tau_max = 2
        adjacency_matrix = construct_adjacency_matrix(response_matrix, null_response_high_percentile, tau_max)
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        for i in range(batch_size):
            if (not Z_gt or not G) == False:
                dm_method= self.model_step((X[i],G,Z_gt[i], spatial_factors[i]))
                mcc, p = compute_mcc(Z_temp, Z_gt[i].detach().cpu().numpy().T, correlation_fn = 'Pearson')
                Z_varimax.append(Z_temp[p,:])
                MCC += mcc
            else:
        ...
        G_latent = pcmci(Z_v, self.model.lag, pc_alpha=1e-2)
        G_latent = torch.tensor(G_latent, device=self.device)
        # self.test_loss(loss)
        if (not Z_gt or not G) == False:
            self.test_f1(G_latent, G)
